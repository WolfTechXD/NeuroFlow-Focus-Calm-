...
const Login: React.FC = () => {
    ...
    const [googleAuthDisabled] = useState(true);

    const selectedPlan = (location.state as any)?.plan;
    ...
    const handleGoogleSignIn = async () => {
        if (googleAuthDisabled) {
            setError('Google sign-in is not yet configured. Please use email/password to create an account or sign in, or try Demo Mode to explore the app.');
            return;
        }
                ...
                <div className="bg-white bg-opacity-90 backdrop-blur-sm rounded-3xl shadow-2xl p-8">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-600">
                            {isSignUp ? 'Join NeuroFlow today' : 'Sign in to continue'}
                        </p>
                    </div>
                    ...
                    <div className="relative">
                        <button
                            ...
                            disabled={loading || googleAuthDisabled}
                            className={`w-full bg-white border-2 text-gray-700 font-semibold py-3 px-4 rounded-xl transition-all flex items-center justify-center gap-3 mb-6 ${
                                googleAuthDisabled
                    ...
                    <form onSubmit={handleEmailAuth} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 text-white font-bold py-3 px-4 rounded-xl hover:shadow-lg transform hover:scale-105 transition-all disabled:opacity-50 disabled:transform-none"
                        >
